...
App.IndexView = Ember.View.extend({
	...
	didInsertElement: function() {
		...
		return (function(){
			...
			var el = document.querySelector(self.get("el"));

			$(window).load(function(){

				window.loaded = true;
				preloader.makeFullScreenViewport(el);
			...
			if (window.loaded){
				...
				preloader.loadedFadeOut(el);
				setTimeout( function(){ preloader.loadedFadeIn(el); preloader.makeFullScreenViewport(el); }, 500);
	...
	didInsertElement: function() {
		...
		return (function(){
			...
			if (window.loaded){
				...
				preloader.loadedFadeOut(el);
				setTimeout(function(){ preloader.loadedFadeIn(el) }, 2000);
...
App.PreloaderAnimation = Ember.Object.extend({
	...
	// OnLoad events: get DOM stuff, if home, resize and preload then fadeIn, if not, just preload and fade In
	// OnClick events: fadeOut current view preload and fade In next view
	// OnResize events: recalc div height and let flexbox do its thing
	makeFullScreenViewport: function(el){

		var vHeight, difference

		vHeight = window.innerHeight;
		difference = vHeight * 0.8;


		el.style.height = difference + 'px';
